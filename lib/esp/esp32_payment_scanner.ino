/*
 * EVSU Canteen IoT - ESP32 RFID Reader with BLE
 * This code provides RFID reading functionality via BLE:
 * 1. Start BLE server with custom service
 * 2. Wait for Flutter app to connect via BLE
 * 3. Receive scanner commands from Flutter app via BLE characteristics
 * 4. Read RFID cards and send data via BLE notifications
 * 5. Flutter app handles all payment processing
 * 
 * Hardware Requirements:
 * - ESP32 Development Board (with BLE support)
 * - MFRC522 RFID Reader (RC522)
 * 
 * MFRC522 to ESP32 Standard SPI Wiring:
 * MFRC522    ESP32
 * --------   -----
 * VCC     -> 3.3V
 * GND     -> GND
...
// IMPORTANT: Make sure ESP32 board is selected in Arduino IDE
